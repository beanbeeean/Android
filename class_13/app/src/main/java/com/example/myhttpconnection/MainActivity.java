        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    // 결과값 받기
                    // 1. 응답 코드 받기
                    // 상태코드 1xx(진행중) , 2xx(성공), 3xx(리다이렉트), 4xx(실패:잘못된 요청), 5xx(연산오류)
                    // GET 성공 -> 200 호출
                    // POST 성공 -> 201 호출
                    ...
                    if(conn.getResponseCode() == HttpURLConnection.HTTP_OK){
                        ...
                        Log.d(TAG,todo.getTitle()); // -> 주소값 ::
                        Log.d(TAG,todo.getBody()); // -> 주소값 ::
...
//                        Log.d(TAG,"value : " + value);
        ...
        }).start(); // Thread는 마지막에 .start 해야 실행 됨
    }
}